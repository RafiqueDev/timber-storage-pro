...
const txn = db.transaction(() => {
  ...
  db.prepare(
    `INSERT INTO companies (id, name, currency, currency_symbol, invoice_prefix, date_format, timezone, low_stock_threshold, next_invoice_seq)
     VALUES ('default', 'Timber Storage Pro', 'PKR', 'Rs.', 'INV', 'DD-MM-YYYY', 'Asia/Karachi', 25, 1)`
  ).run();
  ...
  const warehouses = [
    ...
    const id = nanoid();
    db.prepare(
      "INSERT INTO warehouses (id, company_id, branch_name, branch_address, location, contact_number) VALUES (?, 'default', ?, ?, ?, ?)"
    ).run(id, w.name, w.address, w.location, w.contact);
    return { id, ...w };
  });
  ...
  const parties = ["ABC Timber", "XYZ Wood Traders", "City Timber", "Riverside Lumber Co."].map((name, i) => {
    const id = nanoid();
    db.prepare(
      "INSERT INTO parties (id, party_name, contact_person, phone, address) VALUES (?, ?, ?, ?, ?)"
    ).run(id, name, `Contact Person ${i + 1}`, `030${i}1234567`, `Business District, ${warehouses[i % warehouses.length].location}`);
    return { id, name };
  });
  ...
  for (const wh of warehouses) {
    for (let i = 0; i < 3; i++) {
      ...
      db.prepare(
        `INSERT INTO containers (id, warehouse_id, party_id, container_number, date_of_unloading, initial_packets, loaded_packets, rent_type, rent_rate, status)
         VALUES (?, ?, ?, ?, ?, ?, 0, ?, ?, 'Active')`
      ).run(id, wh.id, party.id, `CONT-${String(cNum).padStart(3, "0")}`, iso(unloadDaysAgo), initial, rentType, rate);
  ...
  for (const c of containers.filter((_, idx) => idx % 3 === 0)) {
    ...
    db.prepare(
      `INSERT INTO loading_logs (id, container_id, warehouse_id, party_id, date_of_loading, packets_loaded, vehicle_number, driver_number, description, created_by)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
    ).run(nanoid(), c.id, c.warehouseId, c.partyId, iso(5), loaded, "TLB-4521", "0301-2223334", "Regular dispatch", admin.id);
  ...
  db.prepare(
    `INSERT INTO invoices (id, invoice_number, warehouse_id, party_id, invoice_date, billing_period_start, billing_period_end, subtotal, paid_amount, balance, status, created_by)
     VALUES (?, 'INV-2026-000001', ?, ?, ?, ?, ?, ?, ?, ?, 'Partially Paid', ?)`
  ).run(invoiceId, container.warehouse_id, container.party_id, iso(2), container.date_of_unloading, iso(2), rent, rent * 0.4, rent * 0.6, admin.id);
  db.prepare(
  ...
  db.prepare(
    `INSERT INTO payments (id, invoice_id, party_id, warehouse_id, amount, payment_date, payment_method, reference, received_by)
     VALUES (?, ?, ?, ?, ?, ?, 'Bank Transfer', 'TRX-9911', ?)`
  ).run(nanoid(), invoiceId, container.party_id, container.warehouse_id, rent * 0.4, iso(1), admin.id);
  db.prepare("UPDATE companies SET next_invoice_seq = 2 WHERE id='default'").run();
